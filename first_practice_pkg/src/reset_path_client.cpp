...
int main(int argc, char **argv)
{
    ...
    // 노드 생성 (클라이언트 노드 이름)
    std::shared_ptr<rclcpp::Node> node = rclcpp::Node::make_shared("reset_path_service_client");
    ...
    // 2. 요청 메시지 생성
    auto request = std::make_shared<first_practice_pkg::srv::ResetPath::Request>();
    ...
    if (rclcpp::spin_until_future_complete(node, result) == 
        rclcpp::FutureReturnCode::SUCCESS)
        {
            ...
            RCLCPP_INFO(node->get_logger(), "Message: %s", response->message.c_str());        } else {
