...
public:
  ...
  explicit DistTurtleActionClient(
  ...
  void send_goal() {
    ...
    RCLCPP_INFO(this->get_logger(), "Sending goal request: Dist %f",
                goal_msg.dist);
...
private:
  ...
  // 1. 서버가 목표를 수락했는지 확인
  void
  goal_response_callback(const GoalHandleDistTurtle::SharedPtr &goal_handle) {
    if (!goal_handle) {
  ...
  void feedback_callback(
    ...
    RCLCPP_INFO(this->get_logger(), "Feedback: Remained Dist = %f m",
