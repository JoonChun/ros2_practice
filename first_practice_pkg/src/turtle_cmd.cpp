...
class TurtleCmdPublisher : public rclcpp::Node
{
    public:
        ...
        {
            // 1. Publisher 생성 : '/turtle1/cmd_vel' 토픽에 geometry_msgs::msg::Twist 메시지 타입으로 퍼블리셔 생성
            publisher_ = this->create_publisher<geometry_msgs::msg::Twist>("/turtle1/cmd_vel", 10);

            // 2. 100ms(0.1초)마다 time_callback 함수 실행 (10Hz)
            timer_ = this->create_wall_timer(100ms, std::bind(&TurtleCmdPublisher::timer_callback, this));
        }
