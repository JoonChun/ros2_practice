...
public:
  ...
  explicit DistTurtleActionServer(
...
private:
  ...
  handle_goal(const rclcpp_action::GoalUUID &uuid,
    ...
    RCLCPP_INFO(this->get_logger(), "Received goal request: move %f meteres",
                goal->dist);
    (void)uuid;
  ...
  handle_cancel(const std::shared_ptr<GoalHandleDistTurtle> goal_handle) {
  ...
  void
  handle_accepted(const std::shared_ptr<GoalHandleDistTurtle> goal_handle) {
    using namespace std::placeholders;
    // execute 함수를 별도 스레드에서 실행 (오래걸리는 작업이므로 블로킹 방지)
    std::thread{std::bind(&DistTurtleActionServer::execute, this, _1),
                goal_handle}
        .detach();
  }
  ...
  void execute(const std::shared_ptr<GoalHandleDistTurtle> goal_handle) {
    ...
    while (total_dist < goal->dist && rclcpp::ok()) {
      ...
      RCLCPP_INFO(this->get_logger(), "Feedback: Remained %f m",
    ...
    if (rclcpp::ok()) {
      ...
      RCLCPP_INFO(this->get_logger(),
                  "Goal succeeded: Moved %f m in %f seconds",
                  result->total_dist, result->total_time);
    }
  }
};
